...
class RelationFormer(nn.Module):
    ...
    def __init__(self, encoder, decoder, config):
        ...
        self.num_queries = (
            ...
            + config.MODEL.DECODER.DUMMY_TOKEN
        )
        ...
        self.class_embed = nn.Linear(
            config.MODEL.DECODER.HIDDEN_DIM, config.MODEL.NUM_CLASSES + 1
        )
        self.bbox_embed = MLP(
        ...
        if config.MODEL.DECODER.RLN_TOKEN > 0:
            self.relation_embed = MLP(
                ...
                config.MODEL.NUM_EDGE_CLASSES + 1,
                3,
            )
        else:
            self.relation_embed = MLP(
...
def build_relationformer(config, **kwargs):
    ...
    model = RelationFormer(encoder, decoder, config, **kwargs)
